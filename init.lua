...
-- ║      waydoku - init.lua     ║
-- ╚═══════════════════════════════╝
-- Press F8 to toggle. wasd move, 1-9 place, 0 erase, r new game.
...
local function close_all()
    ...
    if title_handle then
        title_handle:close(); title_handle = nil
    end
    if status_handle then
        status_handle:close(); status_handle = nil
    end
end
...
local function render_separators()
    ...
    for _, col in ipairs({ 3, 6 }) do
        ...
        for row = 1, 9 do
            ...
            table.insert(sep_handles, waywall.text("|", {
                x = sx, y = sy, size = cfg.size, color = "#888899"
            }))
        end
    end
    -- horizontal --- after row 3 and row 6
...
local function full_render()
    ...
    render_separators()
    render_cells()
    render_status("wasd:move  1-9:place  0:erase  r:new  F8:quit")
end
...
local function check_win()
    for r = 1, 9 do
        for c = 1, 9 do
            if user_grid[r][c] ~= solution[r][c] then return false end
        end
    end
    return true
end
...
local function new_game()
    ...
    for r = 1, 9 do
        given[r] = {}
        for c = 1, 9 do given[r][c] = puzzle[r][c] ~= 0 end
    end
    full_render()
end
    ...
    elseif key == "r" then
        ...
        if num and num >= 1 and num <= 9 then
            if not given[cursor_r][cursor_c] then
                ...
                -- only count as mistake if placing a WRONG number
                -- and it wasn't already wrong
    ...
    for key, func in pairs(config.actions) do
        ...
        for _, k in ipairs(keys_to_capture) do
            if norm == k:lower() then
                is_capture = true; break
            end
        end
        if is_capture then
            config.actions[key] = function()
                if sudoku_on then return update(norm) else return func() end
            end
            saved[norm] = true
        else
            config.actions[key] = function()
                if sudoku_on then return false else return func() end
            end
        end
    end
    for _, k in ipairs(keys_to_capture) do
        ...
        if not saved[norm] then
            config.actions["*-" .. k] = function()
                if sudoku_on then return update(norm) end
                return false
            end
        end
    end
...
local function toggle()
    if not sudoku_on then
        sudoku_on = true
        if not puzzle then new_game() else full_render() end
    else
        sudoku_on = false
        close_all()
    end
...
M.setup = function(config)
    ...
    sudoku_on = false
    setup_keys(config)
    config.actions[cfg.start_key] = function() toggle() end
end
